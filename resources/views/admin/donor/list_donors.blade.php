...
@endif
	...
	<td>
		...
		<a href="{{URL('edit_donors')}}/{{$value->id}}" class="btn btn-primary status_btn" style="background: blue;">edit</a>
		@endif
		@if((Auth::user()->type=='admin') || ($provider1::check_list_permission('delete',Auth::user()->rights)==true))
		<a href="{{URL('delete_donors')}}/{{$value->id}}" id='{{$value->id}}' class="btn btn-info status_btn click-off" style="background:orange;">delete</a>
		@endif
	</td>
